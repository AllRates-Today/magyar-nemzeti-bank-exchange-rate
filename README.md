# Magyar Nemzeti Bank Exchange Rates API — magyar-nemzeti-bank-exchange-rate

[![npm version](https://img.shields.io/npm/v/magyar-nemzeti-bank-exchange-rate.svg)](https://www.npmjs.com/package/magyar-nemzeti-bank-exchange-rate)
[![license](https://img.shields.io/npm/l/magyar-nemzeti-bank-exchange-rate.svg)](https://github.com/AllRates-Today/magyar-nemzeti-bank-exchange-rate/blob/main/LICENSE)
[![zero dependencies](https://img.shields.io/badge/dependencies-0-brightgreen.svg)](https://www.npmjs.com/package/magyar-nemzeti-bank-exchange-rate)
[![TypeScript](https://img.shields.io/badge/TypeScript-types%20included-3178C6.svg)](https://www.typescriptlang.org/)

**Official Magyar Nemzeti Bank (Hungary) daily exchange rates for Node.js and TypeScript. The published central bank rates behind tax filings, customs valuations, audits, and compliant invoicing — not market estimates, but the numbers Magyar Nemzeti Bank itself prints, every business day.**

## 🚀 Why this client?

- 🏛️ **Official published rates** — Magyar Nemzeti Bank's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Magyar Nemzeti Bank itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Magyar Nemzeti Bank table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/mnb?source=EUR&target=HUF"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/mnb').then((x) => x.json());
...
```bash
yarn add magyar-nemzeti-bank-exchange-rate
```

```bash
pnpm add magyar-nemzeti-bank-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/magyar-nemzeti-bank-exchange-rate`](https://www.npmjs.com/package/@allratestoday/magyar-nemzeti-bank-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Magyar Nemzeti Bank rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'magyar-nemzeti-bank-exchange-rate';
...
```js
import { getRatesForDate } from 'magyar-nemzeti-bank-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'EUR', target: 'HUF' });
```
...
- Publisher's own page: [Official exchange rates](https://www.mnb.hu/en/arfolyamok) · [www.mnb.hu](https://www.mnb.hu)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Magyar Nemzeti Bank rates page](https://allratestoday.com/central-bank-rates-api/mnb/)
...
- Magyar Nemzeti Bank quotes **HUF per 1 unit of foreign currency** (e.g. `base: "USD", quote: "HUF"` means HUF per one US dollar).
...
Loading the official Magyar Nemzeti Bank rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/mnb/latest?format=xml&api_key=art_live_..."
```
...
If Magyar Nemzeti Bank does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via HUF from two published rates |
...
| `404` | Pair or date range not covered by Magyar Nemzeti Bank |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'magyar-nemzeti-bank-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('magyar-nemzeti-bank-exchange-rate');

getRate('EUR', 'HUF', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Magyar Nemzeti Bank rates page](https://allratestoday.com/central-bank-rates-api/mnb/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/magyar-nemzeti-bank-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/magyar-nemzeti-bank-exchange-rate)
